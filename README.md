...
| `delete … DOCPACK` | Frees FAT12 clusters so the larger uncompressed `kernel.mnt` fits |
...
| `replace` fails (not enough space / write refused) | Always `cow` to `tmp_images/`, then `delete DOCPACK`, then `replace`. Never mutate `kolibrios-*.img` |

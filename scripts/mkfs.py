...
DEFAULTS = (("exfat", "128M"), ("ntfs", "128M"))


def mkfs_one(fs: str, size: str, *, force: bool = False) -> None:
    fs = fs.lower()
    if fs not in SCRIPTS:
        raise SystemExit(f"ERROR: unsupported filesystem {fs!r} (expected exfat or ntfs)")
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("fs", nargs="?", help="Filesystem: exfat or ntfs")
    parser.add_argument("size", nargs="?", help="Size, e.g. 128M (default)")

...
```powershell
cargo run --manifest-path tools/orch/Cargo.toml -- testdisk
cargo run --manifest-path tools/orch/Cargo.toml -- testdisk --force
```
...
missing or invalid (`[testdisk]` in `project/build.toml`).
...
```powershell
cargo run --manifest-path tools/orch/Cargo.toml -- run
```

Reference floppy boot (also attaches the exFAT disk):

```powershell
cargo run --manifest-path tools/orch/Cargo.toml -- ref
```

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; extsoak — MENUET01 disposable EXT SetFileInfo → GetFileInfo oracle
;;
;; Host/test tooling only. Does not change production FS semantics.
;;
;; Evidence log ESFI.LOG is written on the EXT CoW under test
;; (``/hd0/1/ESFI.LOG``), NOT on the floppy and NOT as ROOT.TXT, so the
;; target inode-12 metadata oracle stays separable from log side-effects.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
; Report layout v2 (binary LE) — keep in sync with host parser.
;   0   magic 'ESFI'
;   4   version = 2
;   8   flags
...
; 140   run_id (host-patched placeholder)
; 144   create_log_eax
; 148   write_log_eax
; 152   target_inode_hint (=12)
; 156   path_tag 'ROOT'
; 160   ticks (sysfn 3 GetTime)
; 164   end
...
; Host patches this dword in the EXTSOAK1 binary before put/boot.
run_id_placeholder:
        dd      0xDEADBEEF

start:
        ; Host patches run_id_placeholder before put; fall back is 0xDEADBEEF.
        mov     eax, [run_id_placeholder]
        mov     [report_run_id], eax
        ; Sidecar uniqueness: sysfn 3 (GetTime) packed clock value.
        mov     eax, 3
        int     0x40
        ...
        ; --- SetFileInfo (primary requested times) ---
        mov     edi, setbuf
        xor     eax, eax
        mov     ecx, 32/4
        rep stosd
...
@@:
        ...
        ; Restore primary times so on-disk host oracle matches requested pair.
        mov     edi, setbuf
        xor     eax, eax
        mov     ecx, 32/4
        rep stosd
...
.log_fail:
        call    board_log_fail
        ; Primary metadata oracle still ran; do not clear FLAG_PASS, but
        ; FLAG_LOG_OK stays clear so host can classify log persistence.
        jmp     .finish
...
; Write durable ESFI.LOG onto EXT CoW (/hd0/1/ESFI.LOG).
; Returns EAX=0 on create+write success.
write_report:
        mov     dword [report_magic], 'ESFI'
        mov     dword [report_version], 2
        mov     dword [report_target_inode], 12
        ...
        ; Create may return "already exists" on repeat CoW reuse — still write.
        mov     eax, 70
        ...
        test    eax, eax
        jnz     .fail
        ; Re-write once more so the on-disk file contains final create/write eax
        ; fields (first write may have raced before those dwords were set).
        mov     eax, 70
        mov     ebx, file70_write_log
        int     0x40
        mov     [report_write_eax], eax
        ...
        ; Rewrite with FLAG_LOG_OK set in the payload.
        mov     eax, 70
        mov     ebx, file70_write_log
        int     0x40
        mov     [report_write_eax], eax
...
file70_set_miss:
        dd 6, 0, 0, 0, setbuf
        db '/hd0/1/NO_SUCH_EXTSOAK.TXT',0
align 4
; Durable log on the EXT CoW under test (8.3 name).
file70_create_log:
        dd 2, 0, 0, 0, 0
        db '/hd0/1/ESFI.LOG',0
align 4
...
msg_start    db 'EXTSOAK START',13,10,0
msg_set      db 'EXTSOAK SET',13,10,0
msg_pass     db 'EXTSOAK PASS',13,10,0
msg_fail     db 'EXTSOAK FAIL',13,10,0
msg_log_ok   db 'EXTSOAK LOG',13,10,0
msg_log_fail db 'EXTSOAK LOGFAIL',13,10,0
msg_imm      db 'EXTSOAK IMM ',0
msg_fin      db 'EXTSOAK FIN ',0
msg_crlf     db 13,10,0

...
(Cuts A–AD). Configuration: [`config.toml`](config.toml).
...
independent production gate under `[[rust.migrations]]` (Cuts A–AD; currently
33 gated symbols plus the Phase C probe blob). Each migration maps `blob` →
...
Latest production checkpoint: Cut AD (`is_protective_mbr`) —
[`docs/migration/cut-ad-implementation.md`](../../docs/migration/cut-ad-implementation.md)
(after validation). Prior: Cut AC (`ipv4_route`).

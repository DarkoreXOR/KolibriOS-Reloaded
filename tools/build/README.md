...
| Command | Meaning |
|---------|---------|
| `run` / `qemu` | Rust blobs → `kernel.mnt` → fresh `build/test/*.img` → QEMU (+ exFAT testdisk) |
| `ref` / `original` | Boot immutable reference `.img` only (`-snapshot`, no rebuild; + exFAT testdisk) |
| `image` | Build + package only |
| `build` | Rust blobs + FASM assemble only |
| `testdisk` | Ensure / recreate `tools/testdata/kolibrios-exfat-test.img` (`--force` to rebuild) |
...
See [`../testdata/README.md`](../testdata/README.md) for the secondary 128 MiB exFAT regression disk.

## Migrations

`config.toml` lists every reloc-free blob under `[[rust.blobs]]` and every
independent production gate under `[[rust.migrations]]` (Cuts A–AH; currently
37 gated symbols plus the Phase C probe blob). Each migration maps `blob` →
...
Latest production checkpoint: Cut AH (`calculate_SetChecksum_field`) —
[`docs/migration/cut-ah-implementation.md`](../../docs/migration/cut-ah-implementation.md)
(after validation). Prior: Cut AG (`ntfs_test_bootsec`).

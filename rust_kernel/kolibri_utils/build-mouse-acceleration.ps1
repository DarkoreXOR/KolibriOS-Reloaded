# Cut L — mouse_acceleration: freestanding build + extract reloc-free blobs.
#
...
#   (+ all prior Cut A–K blobs)
#
# Does NOT assemble kernel.mnt (see docs/migration/cut-l-implementation.md).
...
# that reintroduces relocs into Cut A–K blobs).
...
Write-Host "mouse_acceleration blob:   $(Join-Path $outDir 'rust_mouse_acceleration.bin')"
Write-Host "fat_next_short_name blob:   $(Join-Path $outDir 'rust_fat_next_short_name.bin')"
Write-Host "ntfs_restore_usa blob:      $(Join-Path $outDir 'rust_ntfs_restore_usa.bin')"
Write-Host "ntfs_decode_mcb_entry blob: $(Join-Path $outDir 'rust_ntfs_decode_mcb_entry.bin')"
Write-Host "block_clip blob:            $(Join-Path $outDir 'rust_block_clip.bin')"
...
Write-Host "Next: assemble kernel with USE_RUST_MOUSE_ACCELERATION (see docs/migration/cut-l-implementation.md)"

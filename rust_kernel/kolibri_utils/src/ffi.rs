...
/// `stdcall` rust_fat_get_sector(cluster, sector_ofs, spc, data_start) -> EAX.
///
/// Cut CK: dedicated section for reloc-free extract + FASM `file` embed.
/// Must remain free of GOT/rodata/external calls (verified by extractor).
/// Callee cleans 16 bytes (`ret 16`). FASM trampoline is register-ABI outer
/// (EAX→pair, EBP→FAT*), injects `SECTORS_PER_CLUSTER`/`DATA_START`, and
/// preserves EBX/ECX/EDX/ESI/EDI/EBP (REG-001/011). EAX = absolute sector.
#[no_mangle]
#[link_section = ".text.rust_fat_get_sector"]
pub unsafe extern "stdcall" fn rust_fat_get_sector(
    cluster: u32,
    sector_ofs: u32,
...
/// `stdcall` rust_ext_set_file_info(ctx) → EAX = Kolibri status.
///
/// Cut CV: dedicated section for reloc-free extract + FASM `file` embed.
/// Callee cleans 4 bytes (`ret 4`). Trampoline must call `extfsWritingInit`
/// first (RO non-local return), inject [`ExtSetFileInfoCtx`], and must NOT
/// `add esp` for this arg (REG-009).
///
/// # Safety
/// `ctx` must be a writable [`ExtSetFileInfoCtx`] whose pointers/callbacks
/// are valid; lock must already be held by FASM `extfsWritingInit`.
#[no_mangle]

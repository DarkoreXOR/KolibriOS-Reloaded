# Cut AH — calculate_SetChecksum_field: freestanding build + extract reloc-free blobs.
#
#   powershell -File rust_kernel/kolibri_utils/build-calculate-set-checksum-field.ps1
#
# Produces (kernel currently needs all of these):
#   rust_kernel/kolibri_utils/out/rust_ntfs_test_bootsec.bin
#   (+ all prior Cut A–AG blobs)
#
...
Write-Host "==> extract reloc-free rust_calculate_set_checksum_field"
python $extractGeneric `
    --archive $archive `
    --section ".text.rust_calculate_set_checksum_field" `
    --symbol "rust_calculate_set_checksum_field" `
    --expect-ret-imm 8 `
    --out (Join-Path $outDir "rust_calculate_set_checksum_field.bin")

Write-Host "==> extract reloc-free rust_ntfs_test_bootsec"
python $extractGeneric `
    --archive $archive `
    --section ".text.rust_ntfs_test_bootsec" `
    --symbol "rust_ntfs_test_bootsec" `
    --expect-ret-imm 8 `
    --out (Join-Path $outDir "rust_ntfs_test_bootsec.bin")

Write-Host "==> extract reloc-free rust_ntfs_calculate_time"
python $extractGeneric `
    --archive $archive `
    --section ".text.rust_ntfs_calculate_time" `
    --symbol "rust_ntfs_calculate_time" `
    --expect-ret-imm 4 `
    --out (Join-Path $outDir "rust_ntfs_calculate_time.bin")

Write-Host "==> extract reloc-free rust_ntfs_datetime_to_bdfe"
python $extractGeneric `
    --archive $archive `
    --section ".text.rust_ntfs_datetime_to_bdfe" `
    --symbol "rust_ntfs_datetime_to_bdfe" `
    --expect-ret-imm 12 `
    --out (Join-Path $outDir "rust_ntfs_datetime_to_bdfe.bin")

Write-Host "==> extract reloc-free rust_is_protective_mbr"
python $extractGeneric `
    --archive $archive `
    --section ".text.rust_is_protective_mbr" `
    --symbol "rust_is_protective_mbr" `
    --expect-ret-imm 8 `
    --out (Join-Path $outDir "rust_is_protective_mbr.bin")

Write-Host "==> extract reloc-free rust_ipv4_route"
python $extractGeneric `
    --archive $archive `
    --section ".text.rust_ipv4_route" `
    --symbol "rust_ipv4_route" `
    --expect-ret-imm 36 `
    --out (Join-Path $outDir "rust_ipv4_route.bin")

Write-Host "==> extract reloc-free rust_utf8to16"
python $extractGeneric `
    --archive $archive `
    --section ".text.rust_utf8to16" `
    --symbol "rust_utf8to16" `
    --expect-ret-imm 8 `
    --out (Join-Path $outDir "rust_utf8to16.bin")

Write-Host "==> extract reloc-free rust_pid_to_slot"
python $extractGeneric `
    --archive $archive `
    --section ".text.rust_pid_to_slot" `
    --symbol "rust_pid_to_slot" `
    --expect-ret-imm 12 `
    --out (Join-Path $outDir "rust_pid_to_slot.bin")

Write-Host "==> extract reloc-free rust_is_partition_table_entry"
python $extractGeneric `
    --archive $archive `
    --section ".text.rust_is_partition_table_entry" `
    --symbol "rust_is_partition_table_entry" `
    --expect-ret-imm 16 `
    --out (Join-Path $outDir "rust_is_partition_table_entry.bin")

Write-Host "==> extract reloc-free rust_fix_coff_relocs"
python $extractGeneric `
    --archive $archive `
    --section ".text.rust_fix_coff_relocs" `
    --symbol "rust_fix_coff_relocs" `
    --expect-ret-imm 12 `
    --out (Join-Path $outDir "rust_fix_coff_relocs.bin")

Write-Host "==> extract reloc-free rust_set_io_access_rights"
python $extractGeneric `
    --archive $archive `
    --section ".text.rust_set_io_access_rights" `
    --symbol "rust_set_io_access_rights" `
    --expect-ret-imm 12 `
    --out (Join-Path $outDir "rust_set_io_access_rights.bin")

Write-Host "==> extract reloc-free rust_xfs_get_addr_by_hash"
python $extractGeneric `
    --archive $archive `
    --section ".text.rust_xfs_get_addr_by_hash" `
    --symbol "rust_xfs_get_addr_by_hash" `
    --expect-ret-imm 12 `
    --out (Join-Path $outDir "rust_xfs_get_addr_by_hash.bin")
...
Show-Blob "rust_calculate_set_checksum_field.bin"
Show-Blob "rust_ntfs_test_bootsec.bin"
Show-Blob "rust_ntfs_calculate_time.bin"
Show-Blob "rust_ntfs_datetime_to_bdfe.bin"
Show-Blob "rust_is_protective_mbr.bin"
Show-Blob "rust_ipv4_route.bin"
Show-Blob "rust_utf8to16.bin"
Show-Blob "rust_pid_to_slot.bin"
Show-Blob "rust_is_partition_table_entry.bin"
Write-Host "Next: wire USE_RUST_CALCULATE_SET_CHECKSUM_FIELD trampoline + smoke; default OFF until gates pass"

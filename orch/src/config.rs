//! Configuration for `orch` (`orch/config.toml`).
...
pub struct Config {
    ...
    pub qemu: QemuConfig,
    /// Secondary exFAT IDE disk for filesystem regression tests.
    #[serde(default)]
    pub testdisk: TestdiskConfig,
    #[serde(default)]
...
pub struct RustConfig {
    ...
    /// Completed Cuts A–AH: blob ↔ `USE_RUST_*` gate ↔ kernel include mapping.
    #[serde(default)]
...
/// Persistent 128 MiB exFAT secondary disk attached on every normal QEMU boot.
///
/// Generated by `tools/testdata/create_exfat_test_image.py`; never replaces the
/// boot floppy (`-fda`). See `tools/testdata/README.md`.
#[derive(Debug, Clone, Deserialize)]
pub struct TestdiskConfig {
    /// When true, ensure the image exists and attach it to QEMU (`run`/`qemu`/`ref`).
    #[serde(default = "default_true")]
    pub enabled: bool,
    /// Repo-relative path to the raw exFAT image.
    #[serde(default = "default_testdisk_image")]
    pub image: String,
    /// Repo-relative path to the generator script.
    #[serde(default = "default_testdisk_generator")]
    pub generator: String,
    /// Expected size in MiB (must match the generator).
    #[serde(default = "default_testdisk_size_mib")]
    pub size_mib: u64,
    /// QEMU args template. `{image}` is replaced with the resolved absolute path.
    /// Default attaches as the first IDE hard disk without affecting `-boot a`.
    #[serde(default = "default_testdisk_drive_args")]
    pub drive_args: Vec<String>,
}

impl Default for TestdiskConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            image: default_testdisk_image(),
            generator: default_testdisk_generator(),
            size_mib: default_testdisk_size_mib(),
            drive_args: default_testdisk_drive_args(),
        }
    }
}
...
fn default_testdisk_image() -> String {
    "tools/testdata/kolibrios-exfat-test.img".into()
}

fn default_testdisk_generator() -> String {
    "tools/testdata/create_exfat_test_image.py".into()
}

fn default_testdisk_size_mib() -> u64 {
    128
}

fn default_testdisk_drive_args() -> Vec<String> {
    // index=0 → first IDE HDD (`hd0` in KolibriOS). Floppy remains `-fda` / boot A.
    vec![
        "-drive".into(),
        "file={image},format=raw,if=ide,index=0,media=disk".into(),
    ]
}
...
impl Config {
    ...
    fn validate(&self) -> Result<()> {
        ...
        if self.rust.migrations.is_empty() {
            bail!("config [rust.migrations] is empty — register Cuts A–AH gates");
        }
        ...
        if self.testdisk.enabled {
            if self.testdisk.image.is_empty() {
                bail!("testdisk.image is empty");
            }
            if self.testdisk.generator.is_empty() {
                bail!("testdisk.generator is empty");
            }
            if self.testdisk.size_mib == 0 {
                bail!("testdisk.size_mib must be > 0");
            }
            if self.testdisk.drive_args.is_empty() {
                bail!("testdisk.drive_args is empty");
            }
            if !self
                .testdisk
                .drive_args
                .iter()
                .any(|a| a.contains("{image}"))
            {
                bail!("testdisk.drive_args must contain a `{{image}}` placeholder");
            }
        }
        Ok(())
    }

...
impl<'a> Pipeline<'a> {
    ...
    pub fn ensure_testdisk(&self, force: bool) -> Result<Option<PathBuf>> {
        ...
        if need_create {
            ...
            let mut args = vec![
                ...
                "--size".into(),
                format!("{}M", self.cfg.testdisk.size_mib),
                "-o".into(),
    ...
    pub fn run_qemu(&mut self) -> Result<i32> {
        self.run_qemu_with_opts(&crate::qemu::RunOptions::default())
    }

    pub fn run_qemu_with_opts(&mut self, run_opts: &crate::qemu::RunOptions) -> Result<i32> {
        eprintln!("== QEMU ==");
        eprintln!("Launching...");
        ...
        // Ensure explicit disks exist, or legacy testdisk when no disks specified.
        if run_opts.disks.is_empty() && self.cfg.testdisk.enabled {
            self.ensure_testdisk(false)?;
        } else {
            for disk in &run_opts.disks {
                let path = crate::fs_image::resolve_disk_image(self.root, disk)?;
                if !self.dry_run && !path.is_file() {
                    bail!(
                        "disk image missing for `{disk}`\nExpected: {}\nCreate with: mkfs {disk} 4M",
                        path.display()
                    );
                }
            }
        ...
        let inv = crate::qemu::build_qemu_command(
            self.cfg,
            self.root,
            &image,
            run_opts,
        ...
        let code = run_inherit("QEMU", &inv.executable, &inv.args, self.root, self.dry_run)?;
    ...
    pub fn run_full(&mut self) -> Result<i32> {
        self.run_full_with_opts(&crate::qemu::RunOptions::default())
    }

    pub fn run_full_with_opts(&mut self, run_opts: &crate::qemu::RunOptions) -> Result<i32> {
        self.build_all()?;
        self.create_image()?;
        self.run_qemu_with_opts(run_opts)
    }

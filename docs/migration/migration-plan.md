...
- **Risk:** low.
- **Status (2026-08-09):** **COMPLETE** — Phase C + Cuts A–S production-validated (desktop + network). Bisect log: [`black-screen-investigation.md`](black-screen-investigation.md). Diagnostic smokes D–S re-enabled and validated (Stage 3 smoke pass); Cut M smoke expectation fixed for unsigned `ADD`+`JA`.
...
- **Cut S:** `window._.check_window_position` — **done** 2026-08-09 ([`cut-s-plan.md`](cut-s-plan.md), [`cut-s-implementation.md`](cut-s-implementation.md)). GUI screen-fit; EDI→WDATA.box; display dims via trampoline. **Stop; do not start Cut T.**

...
- **Risk:** low.
- **Status (2026-08-11):** **COMPLETE** — Phase C + Cuts A–AR production-validated (desktop). Bisect log: [`black-screen-investigation.md`](black-screen-investigation.md). Diagnostic smokes D–AR re-enabled and validated (Stage 3 smoke pass); Cut M smoke expectation fixed for unsigned `ADD`+`JA`.
...
- **Cut AR:** `r_f_port_area` — **done** 2026-08-11 ([`cut-ar-plan.md`](cut-ar-plan.md), [`cut-ar-implementation.md`](cut-ar-implementation.md)). I/O port-area reserve/free (syscall 46; Cut X follow-on); Path B after post-AQ Stage-4 audit (paging Path A rejected; Stage-4 leaves exhausted); reloc-free. **Stop; do not start Cut AS.**

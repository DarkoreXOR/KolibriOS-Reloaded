...
- **Risk:** low.
- **Status (2026-08-10):** **COMPLETE** — Phase C + Cuts A–AD production-validated (desktop + network NIC). Bisect log: [`black-screen-investigation.md`](black-screen-investigation.md). Diagnostic smokes D–AD re-enabled and validated (Stage 3 smoke pass); Cut M smoke expectation fixed for unsigned `ADD`+`JA`.
...
- **Cut AD:** `is_protective_mbr` — **done** 2026-08-10 ([`cut-ad-plan.md`](cut-ad-plan.md), [`cut-ad-implementation.md`](cut-ad-implementation.md)). GPT protective-MBR ZF recognition; Path B after cluster audit; reloc-free. **Stop; do not start Cut AE.**

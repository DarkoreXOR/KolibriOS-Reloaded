...
- **Risk:** low.
- **Status (2026-08-10):** **COMPLETE** — Phase C + Cuts A–AG production-validated (desktop + network NIC). Bisect log: [`black-screen-investigation.md`](black-screen-investigation.md). Diagnostic smokes D–AG re-enabled and validated (Stage 3 smoke pass); Cut M smoke expectation fixed for unsigned `ADD`+`JA`.
...
- **Cut AG:** `ntfs_test_bootsec` — **done** 2026-08-10 ([`cut-ag-plan.md`](cut-ag-plan.md), [`cut-ag-implementation.md`](cut-ag-implementation.md)). NTFS bootsector multi-rule CF validate; Path B after post-AF raised-bar cluster audit (no Rust-owned subsystem yet); reloc-free. **Stop; do not start Cut AH.**

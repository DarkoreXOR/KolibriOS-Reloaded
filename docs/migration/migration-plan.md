...
- **Risk:** low.
- **Status (2026-08-09):** **COMPLETE** — Phase C + Cuts A–T production-validated (desktop + network). Bisect log: [`black-screen-investigation.md`](black-screen-investigation.md). Diagnostic smokes D–T re-enabled and validated (Stage 3 smoke pass); Cut M smoke expectation fixed for unsigned `ADD`+`JA`.
...
- **Cut T:** `fsTime2bdfe` — **done** 2026-08-09 ([`cut-t-plan.md`](cut-t-plan.md), [`cut-t-implementation.md`](cut-t-implementation.md)). EDI+=8 calendar inverse; completes G pair. **Stop; do not start Cut U.**

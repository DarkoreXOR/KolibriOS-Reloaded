...
- **Risk:** low.
- **Status (2026-08-10):** **COMPLETE** — Phase C + Cuts A–Z production-validated (desktop + network). Bisect log: [`black-screen-investigation.md`](black-screen-investigation.md). Diagnostic smokes D–Z re-enabled and validated (Stage 3 smoke pass); Cut M smoke expectation fixed for unsigned `ADD`+`JA`.
...
- **Cut Z:** `is_partition_table_entry` — **done** 2026-08-10 ([`cut-z-plan.md`](cut-z-plan.md), [`cut-z-implementation.md`](cut-z-implementation.md)). MBR/EBR partition-table entry validate (Bootable + 64-bit half-capacity); CF-out; reloc-free. **Stop; do not start Cut AA.**

...
**Last inventory baseline:** post–Cut BA (2026-08-11).

**Gates source of truth:** `project/build.toml` `[[rust.migrations]]`
(56 production entries, all `enabled = true` after BA).
...
- [x] `utf8to16` — Cut AB
- [ ] `cp866toUTF8_string` — deferred: unicode string wrapper
- [ ] `uni2ansi_char` — deferred/ban: AN inverse
- [ ] `UTF16to8_string` — deferred: Q string wrapper
...
- [ ] `strrchr` — candidate: string leaf
- [ ] `strtoint_dec` — candidate: decimal parse leaf

## fs/common

- [x] `fsCalculateTime` — Cut G
- [x] `fsTime2bdfe` — Cut T
- [ ] `fsGetTime` — candidate: CMOS/time query; calendar cluster caution
- [ ] `fsReadCMOS` — deferred: CMOS calendar
...
- [ ] `xfs._.conv_time_to_kos_epoch` — deferred: XFS time deepen after AK
- [ ] `xfs._.get_last_dirblock` — deferred: XFS dir deepen

## fs/EXT

- [x] `ext_read_time` — Cut AL
- [ ] `ext_read_all_times` — candidate: AL compose
- [ ] `ext_SetFileInfo` — deferred: FS write path
- [ ] `ext_write_time` — candidate: AL write twin
- [ ] `getInodeLocation` — deferred/ban: AW address-math; no --disk ext

## fs/ISO9660

- [x] `iso9660_compare_name` — Cut AJ
- [ ] `iso9660_copy_name` — thin: name-copy glue after AJ
...
- [ ] `process_partition_table_entry` — deferred: Z sibling orchestration

## blkdev/AHCI

- [x] `ahci_find_cmdslot` — Cut AV
- [ ] `ahci_is_sig_known` — deferred: AV sibling deepen
- [ ] `ahci_port_wait` — deferred: AV Path A reject — controller wait
...
- [ ] `tcp_mss` — deferred: M/V TCP deepen
- [ ] `tcp_outflags` — candidate: AZ #3 state→flags table
...
- [ ] `v86_get_lin_addr` — candidate: V86 linear translate
...
- [ ] `swap_bytes_in_words` — candidate: endian helper
...
- [ ] `window._.set_window_clientbox` — deferred: GUI Stage 7

## video

- [x] `block_clip` — Cut H
- [ ] `blit_32` — deferred: blit hot path
- [ ] `blit_clip` — deferred: Cut H sibling; GUI composition

## hid

- [x] `mouse_acceleration` — Cut L
- [ ] `hotkey_do_test` — candidate: hotkey match
...
- AO calendar siblings (`fat_date_to_bdfe`, `bdfe_to_fat_*`)

...
**LOCAL FACT (Cuts A-CB complete, 2026-08-12):** Stage 2/3/4/5/8 production envelope through Cut CB (`ahci_port_wait`) — see [`migration-plan.md`](migration-plan.md) and durable inventory [`migration-todo.md`](migration-todo.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Fresh post-CA audit rejected Path A; Cut CB is Path B (AHCI TFD poll leaf; MMIO/ticks stay FASM). Cuts BT/BU/BV/BW/BX/BY/BZ/CA closed — do not modify. Cut CB closed — do not modify.

...
**LOCAL FACT (Cuts A–BF complete, 2026-08-12):** Stage 2/3/5 production envelope through Cut BF (`strncpy`) — see [`migration-plan.md`](migration-plan.md) and durable inventory [`migration-todo.md`](migration-todo.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Post-BE audit: Path A rejected; HID deepen / thin P sibling / Stage-4 address math / PE leftovers deferred; Cut BF is Path B (bounded padded string copy; shmem + PE export).

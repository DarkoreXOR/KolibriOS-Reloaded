...
**LOCAL FACT (Cuts A-BP complete, 2026-08-12):** Stage 2/3/4/5/8 production envelope through Cut BP (`UTF16to8_string`) — see [`migration-plan.md`](migration-plan.md) and durable inventory [`migration-todo.md`](migration-todo.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Fresh post-BO audit rejected Path A; Cut BP remains Path B (UTF-16 string streaming with attach-only exFAT A/B; no scripted filename-walk soak).

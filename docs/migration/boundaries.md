...
**LOCAL FACT (Cuts A-BS complete, 2026-08-12):** Stage 2/3/4/5/8 production envelope through Cut BS (`ext_write_time`) — see [`migration-plan.md`](migration-plan.md) and durable inventory [`migration-todo.md`](migration-todo.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Fresh post-BR audit rejected Path A; Cut BS remains Path B (EXT write-time pack after `fsGetTime`; `--disk ext` soak NOT AVAILABLE).

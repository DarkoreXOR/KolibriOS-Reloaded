...
**LOCAL FACT (Cuts A–AF complete, 2026-08-10):** Stage 2 production envelope through Cut AF (`ntfsCalculateTime`) — see [`migration-plan.md`](migration-plan.md). Prefer `tools/build` (`config.toml` blob + migration registry) over manual per-cut scripts for day-to-day builds. Post-AE cluster audit concluded Path B remains appropriate.

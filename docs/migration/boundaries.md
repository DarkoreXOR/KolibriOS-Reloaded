...
**LOCAL FACT (Cuts A-BR complete, 2026-08-12):** Stage 2/3/4/5/8 production envelope through Cut BR (`ext_read_all_times`) — see [`migration-plan.md`](migration-plan.md) and durable inventory [`migration-todo.md`](migration-todo.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Fresh post-BQ audit rejected Path A; Cut BR remains Path B (EXT inode triple-timestamp fan-out; `--disk ext` soak NOT AVAILABLE).

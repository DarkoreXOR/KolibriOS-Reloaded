...
**LOCAL FACT (Cuts A–BB complete, 2026-08-12):** Stage 2/3/5 production envelope through Cut BB (`strrchr`) — see [`migration-plan.md`](migration-plan.md) and durable inventory [`migration-todo.md`](migration-todo.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Post-BA audit: Path A rejected; `strtoint_dec` dead (unlinked); PE/TCP/FAT deepen deferred; Cut BB is Path B (core string reverse search; process-create soak).

...
**LOCAL FACT (Cuts A–AO complete, 2026-08-11):** Stage 2 production envelope through Cut AO (`fat_time_to_bdfe`) — see [`migration-plan.md`](migration-plan.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Post-AN raised-bar cluster audit concluded Path B remains appropriate (DOS packed-time leaf; FAT datetime siblings ≠ Path A; Unicode/ISO/exFAT/XFS/EXT/GUI pairs rejected).

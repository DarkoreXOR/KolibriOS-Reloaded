...
**LOCAL FACT (Cuts A–BM complete, 2026-08-12):** Stage 2/3/4/5/8 production envelope through Cut BM (`ahci_is_sig_known`) — see [`migration-plan.md`](migration-plan.md) and durable inventory [`migration-todo.md`](migration-todo.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Post-BL audit: Path A rejected; Cut BM is Path B (AHCI PxSIG→ZF; complements AV+BG; `--bus ahci` soak; synthetic-signature smoke).

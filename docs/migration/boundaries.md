...
**LOCAL FACT (Cuts A–BL complete, 2026-08-12):** Stage 2/3/4/5/8 production envelope through Cut BL (`v86_get_lin_addr`) — see [`migration-plan.md`](migration-plan.md) and durable inventory [`migration-todo.md`](migration-todo.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Post-BK audit: Path A rejected; PE thin Path B novelty exhausted (Y+AT+BK); Cut BL is Path B (V86 PTE→linear; complements AQ; BIOS `/bd*` soak via `biosdisks=on`; synthetic-PTE smoke).

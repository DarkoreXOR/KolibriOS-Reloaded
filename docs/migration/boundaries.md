...
**LOCAL FACT (Cuts A–AJ complete, 2026-08-10):** Stage 2 production envelope through Cut AJ (`iso9660_compare_name`) — see [`migration-plan.md`](migration-plan.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Post-AI raised-bar cluster audit concluded Path B remains appropriate (ISO leaf; AH+AI helper reuse and ISO compare-pair proximity ≠ Path A).

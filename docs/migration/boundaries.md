...
**LOCAL FACT (Cuts A-BU complete, 2026-08-12):** Stage 2/3/4/5/8 production envelope through Cut BU (`fix_coff_symbols`) — see [`migration-plan.md`](migration-plan.md) and durable inventory [`migration-todo.md`](migration-todo.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Fresh post-BT audit rejected Path A; Cut BU remains Path B (COFF symbol-table resolve; `get_proc_ex` stays FASM via injected callback). Cut BT closed — do not modify.

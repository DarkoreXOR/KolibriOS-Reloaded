...
**LOCAL FACT (Cuts A-BV complete, 2026-08-12):** Stage 2/3/4/5/8 production envelope through Cut BV (`fsGetTime`) — see [`migration-plan.md`](migration-plan.md) and durable inventory [`migration-todo.md`](migration-todo.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Fresh post-BU audit rejected Path A; Cut BV remains Path B (CMOS orchestration; `fsReadCMOS` stays FASM via injected callback). Cut BT/BU closed — do not modify.

...
**LOCAL FACT (Cuts A-BQ complete, 2026-08-12):** Stage 2/3/4/5/8 production envelope through Cut BQ (`cp866toUTF8_string`) — see [`migration-plan.md`](migration-plan.md) and durable inventory [`migration-todo.md`](migration-todo.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Fresh post-BP audit rejected Path A; Cut BQ remains Path B (CP866 string streaming with attach-only exFAT A/B; ISO9660 `--disk` soak NOT AVAILABLE).

...
**LOCAL FACT (Cuts A-BN complete, 2026-08-12):** Stage 2/3/4/5/8 production envelope through Cut BN (`xfs._.conv_time_to_kos_epoch`) — see [`migration-plan.md`](migration-plan.md) and durable inventory [`migration-todo.md`](migration-todo.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Post-BM audit: Path A rejected; Cut BN is Path B (classic XFS BE-seconds DQ -> BDFE; composes Cut T; `--disk xfs` attach-only A/B; synthetic time fixtures in smoke).

...
**LOCAL FACT (Cuts A–BJ complete, 2026-08-12):** Stage 2/3/5 production envelope through Cut BJ (`is_string_userspace`) — see [`migration-plan.md`](migration-plan.md) and durable inventory [`migration-todo.md`](migration-todo.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Post-BI audit: Path A rejected; ISO Path B exhausted (AJ+BI); HID deepen / Stage-4 V86 / PE thin / trivial AHCI sig CMP deferred; Cut BJ is Path B (NUL-scan userspace ZF gate; complements P+AZ; reject-only early-init smoke + host accept corpus).

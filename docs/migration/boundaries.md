...
**LOCAL FACT (Cuts A–BK complete, 2026-08-12):** Stage 2/3/5/8 production envelope through Cut BK (`coff_get_align`) — see [`migration-plan.md`](migration-plan.md) and durable inventory [`migration-todo.md`](migration-todo.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Post-BJ audit: Path A rejected; Stage-3 Path B novelty exhausted (P+AZ+BJ); V86 weak-soak / AHCI-trivial / HID-TCP-XFS deepen deferred; Cut BK is Path B (COFF Characteristics→align mask; complements Y+AT; synthetic-section smoke).

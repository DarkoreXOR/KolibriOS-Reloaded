...
**LOCAL FACT (Cuts A-BW complete, 2026-08-12):** Stage 2/3/4/5/8 production envelope through Cut BW (`fat_date_to_bdfe`) — see [`migration-plan.md`](migration-plan.md) and durable inventory [`migration-todo.md`](migration-todo.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Fresh post-BV audit rejected Path A; Cut BW is Path B (DOS FAT date unpack; AO time sibling). Cuts BT/BU/BV closed — do not modify.

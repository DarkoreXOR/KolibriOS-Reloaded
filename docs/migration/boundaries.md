...
**LOCAL FACT (Cuts A–BI complete, 2026-08-12):** Stage 2/3/5 production envelope through Cut BI (`iso9660_copy_name`) — see [`migration-plan.md`](migration-plan.md) and durable inventory [`migration-todo.md`](migration-todo.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Post-BH audit: Path A rejected; HID deepen / thin P sibling / Stage-4 V86 / PE thin / trivial AHCI sig CMP deferred; Cut BI is Path B (ISO9660 volume-name encoding dispatch; `--disk iso9660` soak PASS; ban/deferred helpers inlined, not cut as symbols).

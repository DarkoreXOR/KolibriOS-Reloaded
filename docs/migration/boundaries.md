...
**LOCAL FACT (Cuts A-BT complete, 2026-08-12):** Stage 2/3/4/5/8 production envelope through Cut BT (`ntfsGetTime`) — see [`migration-plan.md`](migration-plan.md) and durable inventory [`migration-todo.md`](migration-todo.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Fresh post-BS audit rejected Path A; Cut BT remains Path B (NTFS CMOS metadata FILETIME pack after `fsGetTime`; `--disk ntfs` attach-only soak PASS).

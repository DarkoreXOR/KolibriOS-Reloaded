...
**LOCAL FACT (Cuts A–BC complete, 2026-08-12):** Stage 2/3/5 production envelope through Cut BC (`fat_name_is_legal`) — see [`migration-plan.md`](migration-plan.md) and durable inventory [`migration-todo.md`](migration-todo.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Post-BB audit: Path A rejected; `strchr` export-only; PE/TCP/string deepen deferred; Cut BC is Path B (FAT LFN charset validate; create-path leaf).

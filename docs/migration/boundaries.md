...
**LOCAL FACT (Cuts A–BH complete, 2026-08-12):** Stage 2/3/5 production envelope through Cut BH (`strlen`) — see [`migration-plan.md`](migration-plan.md) and durable inventory [`migration-todo.md`](migration-todo.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Post-BG audit: Path A rejected; HID deepen / ISO glue+ban / trivial AHCI sig CMP / Stage-4 V86 deferred; Cut BH is Path B (C-string length/`scasb`; EXT callers; soak NOT AVAILABLE — no `--disk ext`).

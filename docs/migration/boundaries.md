...
**LOCAL FACT (Cuts A-BY complete, 2026-08-12):** Stage 2/3/4/5/8 production envelope through Cut BY (`bdfe_to_fat_time`) — see [`migration-plan.md`](migration-plan.md) and durable inventory [`migration-todo.md`](migration-todo.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Fresh post-BX audit rejected Path A; Cut BY is Path B (BDFE→DOS FAT time pack; AO read inverse). Cuts BT/BU/BV/BW/BX closed — do not modify. Cut BY closed — do not modify.

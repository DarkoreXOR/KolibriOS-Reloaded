...
**LOCAL FACT (Cuts A–BG complete, 2026-08-12):** Stage 2/3/5 production envelope through Cut BG (`swap_bytes_in_words`) — see [`migration-plan.md`](migration-plan.md) and durable inventory [`migration-todo.md`](migration-todo.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Post-BF audit: Path A rejected; HID deepen / EXT-only strlen / ISO glue+ban / trivial AHCI sig CMP deferred; Cut BG is Path B (endian word-swap; AHCI IDENTIFY model string).

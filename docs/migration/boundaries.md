...
**LOCAL FACT (Cuts A-CA complete, 2026-08-12):** Stage 2/3/4/5/8 production envelope through Cut CA (`fsReadCMOS`) — see [`migration-plan.md`](migration-plan.md) and durable inventory [`migration-todo.md`](migration-todo.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Fresh post-BZ audit rejected Path A; Cut CA is Path B (CMOS BCD decode leaf; port I/O stays FASM). Cuts BT/BU/BV/BW/BX/BY/BZ closed — do not modify. Cut CA closed — do not modify.

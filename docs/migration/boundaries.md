...
**LOCAL FACT (Cuts A-BX complete, 2026-08-12):** Stage 2/3/4/5/8 production envelope through Cut BX (`bdfe_to_fat_date`) — see [`migration-plan.md`](migration-plan.md) and durable inventory [`migration-todo.md`](migration-todo.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Fresh post-BW audit rejected Path A; Cut BX is Path B (BDFE→DOS FAT date pack; BW read inverse). Cuts BT/BU/BV/BW closed — do not modify.

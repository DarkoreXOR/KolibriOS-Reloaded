...
**LOCAL FACT (Cuts A-BO complete, 2026-08-12):** Stage 2/3/4/5/8 production envelope through Cut BO (`xfs._.get_last_dirblock`) — see [`migration-plan.md`](migration-plan.md) and durable inventory [`migration-todo.md`](migration-todo.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Fresh post-BN audit still rejects Path A; Cut BO remains Path B (last-extent dirblock arithmetic with `--disk xfs` attach-only A/B; no scripted XFS browse/inode-walk soak yet).

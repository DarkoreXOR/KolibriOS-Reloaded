...
**LOCAL FACT (Cuts A–BE complete, 2026-08-12):** Stage 2/3/5 production envelope through Cut BE (`hotkey_do_test`) — see [`migration-plan.md`](migration-plan.md) and durable inventory [`migration-todo.md`](migration-todo.md). Prefer `scripts/` + `project/build.toml` blob/migration registry over manual per-cut scripts for day-to-day builds. Post-BD audit: Path A rejected; thin P sibling / Stage-4 address math / PE leftovers deferred; Cut BE is Path B (HID hotkey field match; scancode→hotkey loop).

...
.\target\release\kolibri_img.exe cow ..\..\tmp_images\cut-w-final.img ..\..\tmp_images\cut-x-on.img
.\target\release\kolibri_img.exe replace ..\..\tmp_images\cut-x-on.img KERNEL.MNT ..\..\kernel\bin\kernel.mnt
```
...
Images: CoW from `tmp_images/cut-w-final.img`, replace `KERNEL.MNT`.

| Build | Switch | Desktop | Internet |
|-------|--------|---------|----------|
| OFF | `USE_RUST_SET_IO_ACCESS_RIGHTS=0` | **OK** (QMP `running` + screendump `tmp_images/cut-x-off.ppm`, 779380 non-black samples) | **OK** (e1000 + user net) |
| ON | `USE_RUST_SET_IO_ACCESS_RIGHTS=1` | **OK** (screendump `tmp_images/cut-x-on.ppm`, 779380 non-black samples) | **OK** |
...
Production image: `tmp_images/cut-x-final.img`.
